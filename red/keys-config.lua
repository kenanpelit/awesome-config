...
function hotkeys:init(args)
    ...
    self.alsamix = args.alsamix or "/bin/sh -c ${HOME}/.scripts/start-alsamixer.sh"
    self.buku = args.buku or "/bin/sh -c ${HOME}/.scripts/buku-bookmark.sh"
    self.touchpad = args.touchpad or "/bin/sh -c ${HOME}/.scripts/set-touchpad-toggle.sh"
    self.pulsechange = args.pulsechange or "/bin/sh -c ${HOME}/.scripts/set-pulse-switch-out"
    self.seltr = args.seltr or "/bin/sh -c ${HOME}/.scripts/seltr"
    self.seltrl = args.seltrl or "/bin/sh -c ${HOME}/.scripts/seltr-long"
    self.seltrp = args.seltrp or "/bin/sh -c ${HOME}/.scripts/seltr-long-tr"
    self.weechat = args.weechat or "/bin/sh -c ${HOME}/.scripts/start-weechat.sh"
    self.windows = args.windows or "/bin/sh -c ${HOME}/.scripts/start-windows.sh"
    self.cplay = args.cplay or "/bin/sh -c ${HOME}/.scripts/cplay"
    self.lock = args.lock or "/bin/sh -c ${HOME}/.scripts/start-lock.sh"
    self.mail = args.mail or "/bin/sh -c ${HOME}/.scripts/start-mutt-1.sh"
    --self.emacsnw = args.emacsnw or "/bin/sh -c ${HOME}/.scripts/start-emacs-1.sh"
    self.emacs = args.emacs or "emacs"
    self.gnus = args.gnus or "emacs -f gnus"
    self.mpvf = args.mpvf or "/bin/sh -c ${HOME}/.scripts/series-seinfeld.sh"
    self.mpvm = args.mpvm or "/bin/sh -c ${HOME}/.scripts/series-meet.sh"
    self.off = args.off or "/bin/sh -c ${HOME}/.scripts/start-poweroff.sh"
    self.reboot = args.reboot or "/bin/sh -c ${HOME}/.scripts/start-reboot.sh"
    self.suspend = args.suspend or "/bin/sh -c ${HOME}/.scripts/start-suspend.sh"
    self.hibernate = args.hibernate or "/bin/sh -c ${HOME}/.scripts/start-hibernate.sh"
    self.scrot = args.scrot or "/bin/sh -c ${HOME}/.scripts/start-screenshot.sh"
    self.windowsSave = args.windowsSave or "/bin/sh -c ${HOME}/.scripts/start-windows-save.sh"
    self.virtualbox = args.virtualbox or "virtualbox"
    self.lightu = args.lightu or "/bin/sh -c ${HOME}/.scripts/qlight --up"
    self.lightd = args.lightd or "/bin/sh -c ${HOME}/.scripts/qlight --down"
    self.sessionr = args.sessionr or "/bin/sh -c ${HOME}/.scripts/session-r.sh"
    self.sessions = args.sessions or "/bin/sh -c ${HOME}/.scripts/session-s.sh"
    ...
    self.raw_global = {
        ...
        {
            args = { {             "Mod5" }, "a", function () awful.util.spawn(self.alsamix) end },
            comment = "Alsamixer"
        },
        {
